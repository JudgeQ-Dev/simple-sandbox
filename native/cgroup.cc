...
CgroupInfo::CgroupInfo(const string &controller, const string &group) : Controller(controller), Group(group) {
...
map<string, vector<fs::path>> InitializeCgroup() {
    ...
    while ((ent = getmntent_r(proc_mount.get(), temp_ent.get(), buf, sizeof(buf))) != NULL) {
...
static fs::path EnsureGroup(const CgroupInfo &info) {
    ...
    if (!fs::exists(groupDirectory) || !fs::is_directory(groupDirectory)) {
        throw std::runtime_error(
                (format("Path {} is not valid (does not exist or is not a directory).", groupDirectory)));
    }

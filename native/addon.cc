...
Napi::Value NodeGetCgroupProperty2(const Napi::CallbackInfo &info) {
    ...
    try {
        ...
        Napi::Error::New(env, "Something unexpected happened while getting cgroup property.")
...
std::vector<string> StringArrayToVector(const Napi::Array &array) {
    ...
    for (size_t i = 0; i < array.Length(); i++) {
        result[i] = GetStringWithEmptyCheck(array[i]);
    }
...
Napi::Value NodeStartSandbox(const Napi::CallbackInfo &info) {
    ...
    param.memoryLimit = jsparam.Get("memory").ToNumber().Int64Value() / 4 *
                        5;  // Reserve some space to detect memory limit exceeding.
